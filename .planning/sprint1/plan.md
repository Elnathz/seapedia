...
For T8, T9, T10, and the demo, after assets build (`sail npm run dev`), drive the app via the
Playwright MCP server (`mcp__plugin_playwright_playwright__*` tools):

1. Navigate to each new/changed page (landing, catalog, product detail, login, register, each role
   dashboard, the review page).
2. Screenshot at **360px, 768px, 1280px, and 1920px** (TDD §11 responsive targets + the desktop
   resolution the competition demo runs at) — confirm no overflow/broken layout; mobile shows
   `BottomNav`, desktop shows sidebar.
